...
HXDLIN( 654)						if (point->_weak) {
HXLINE( 654)							point->put();
            						}
HXDLIN( 654)						if (contained) {
HXLINE( 656)							this->_point = touch->getWorldPosition(camera,this->_point);
HXLINE( 658)							 ::flixel::math::FlxBasePoint point = this->_point;
...
HXLINE( 658)								_hx_tmp = event->sprite->pixelsOverlapPoint(point,1,camera);
            							}
            							else {
HXLINE( 658)								_hx_tmp = event->object->overlapsPoint(point,true,camera);
            							}
